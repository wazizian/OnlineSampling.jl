...
plot!(
    [x_pos[i][1] for i = 1:block:length(obs)],
    [traj_speed[i] for i = 1:block:length(obs)],
    label = "truth",
)
...
@gif for (i, cloud) in enumerate(cloud_sbp_iter)
    ...
    ylims!((0.0, 2))
    p = plot!(
        [x_pos_utmb.(x_pos[(i-1)*block+1]); x_pos_utmb.(x_pos[(i-1)*block+1])],
        [alt[(i-1)*block+1]; 0],
        lw = 2,
        lc = "red",
        legend = false,
...
@gif for (i, cloud) in enumerate(cloud_sbp_iter)
    ...
    ylims!((0.0, 1.3))
    p = plot!(
        [polar_x_pos.(x_pos[(i-1)*block+1]); polar_x_pos.(x_pos[(i-1)*block+1])],
        [alt[(i-1)*block+1]; 0],
        lw = 2,
        lc = "red",
        proj = :polar,
