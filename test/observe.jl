...
@testset "invalid obs : atan" begin
    ...
    @test_throws OnlineSampling.UntrackedObservation @noderun T = 3 f(obs)
    @test_throws OnlineSampling.UntrackedObservation @noderun particles = 1 algo=delayed_sampling T = 3 f(
        obs,
    )
    @test_throws OnlineSampling.UntrackedObservation @noderun particles = 1 algo=belief_propagation T = 3 f(
        obs,
    )
end
...
@testset "invalid obs : atan bis" begin
    ...
    @noderun T = 3 f(obs)
    @test_throws OnlineSampling.RealizedObservation @noderun particles = 1 algo=delayed_sampling T = 3 f(
        obs,
    )
    @test_throws OnlineSampling.RealizedObservation @noderun particles = 1 algo=belief_propagation T = 3 f(
        obs,
    )
end
