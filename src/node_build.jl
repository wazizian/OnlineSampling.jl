...
function treat_node_calls(state_symb::Symbol, reset_symb::Symbol, body::Expr)
    ...
    new_body = prewalk(body) do ex
        @capture(ex, (@node cond_ particles=val_ f_(args__)) | 
                     (@node particles=val_ f_(args__)) | 
                     (@node cond_ f_(args__)) |
                     (@node f_(args__))) || return ex
        ...
        if node_particles > 0
            return quote
                ...
                begin
                    ...
                    @node $(cond) $(@__MODULE__).smc($(node_particles), $(mem_struct_type), $(f), $(args...))
                end
            end
        end
        # get new id
        ...
        update_score = quote
            $(state_symb).loglikelihood += $(@__MODULE__).OnlineSMC.loglikelihood($(state_symb).$(call_id))
        end
