...
CdMvNormal(linear::Linear, μ::Mean, Σ::Cov) where {Linear <: StridedMatrix, Mean, Cov <: AbstractPDMat} = CdMvNormal{Linear, Mean, Cov}(linear, μ, Σ)
CdMvNormal(linear::Linear, μ::Mean, Σ::Cov) where {Linear <: AbstractArray, Mean, Cov <: AbstractArray} = CdMvNormal(Array(linear), μ, PDMat(Array(Σ)))
