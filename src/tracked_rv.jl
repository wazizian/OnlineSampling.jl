...
"""
abstract type AbstractTrackedRV{T, F, S, D<:Distribution{F,S}} <: AbstractTrackedObservation{T, F, S, D} end
...
"""
struct LinearTracker{T<:AbstractVector, Linear <: AbstractMatrix, D<:Distribution{Multivariate, Continuous}} <: AbstractTrackedRV{T, Multivariate, Continuous, D}
    gm::DS.GraphicalModel
    id::Int64
    linear::Linear
    offset::T
...
track_rv(gm::DS.GraphicalModel, d::AbstractMvNormal) = LinearTracker{AbstractVector, AbstractMatrix, typeof(d)}(gm, DS.initialize!(gm, d))
# TODO (api impr): clean the two following lines
track_rv(gm::DS.GraphicalModel, t::Tuple{DS.CdMvNormal, Int64}) = LinearTracker(gm, DS.initialize!(gm, t...))

"""
    Compute the conditional MvNormal distribution from a LinearTracker
"""
# TODO (api impr): clean the lines
Distributions.MvNormal(μ::LinearTracker{T, Linear, D}, cov) where {T, Linear, D <: AbstractMvNormal} =
(DS.CdMvNormal(μ.linear, μ.offset, cov), μ.id)
